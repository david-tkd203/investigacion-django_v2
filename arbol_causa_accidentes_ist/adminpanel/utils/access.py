...
User = get_user_model()
SUPER_ROLES = {ROLE_SUPER_ADMIN, ROLE_ADMIN_IST}
...
def usuarios_permitidos_para_asignar(
    ...
    if rol == ROLE_ADMIN_HOLDING:
        hid = holding_id or getattr(user, "holding_id", None)
        qs = qs.filter(holding_id=hid, rol__in=[ROLE_ADMIN_HOLDING, ROLE_ADMIN_EMPRESA, ROLE_INVESTIGADOR])
    ...
    if rol == ROLE_ADMIN_EMPRESA:
        eid = getattr(user, "empresa_id", None)
        qs = qs.filter(empresa_id=eid, rol__in=[ROLE_ADMIN_EMPRESA, ROLE_INVESTIGADOR])
